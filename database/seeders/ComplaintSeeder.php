...
class ComplaintSeeder extends Seeder
{
    public function run(): void
    {
        ...
        foreach ($fixed as [$customerId, $title, $category, $priority, $status]) {
            $this->createComplaint(Customer::find($customerId), $title, $category, $priority, $status, $cs, 4);
        }

        Customer::where('status_id', 'AK')->inRandomOrder()->limit(120)->get()->each(function (Customer $customer, int $i) use ($categories, $statuses, $cs) {
            $status = $statuses[$i % count($statuses)];
            $priority = ['low', 'normal', 'high', 'urgent'][$i % 4];
            $title = "{$categories[$i % count($categories)]} - laporan {$customer->id}";
            $this->createComplaint($customer, $title, $categories[$i % count($categories)], $priority, $status, $cs, $i % 3);
        });
    }

    private function createComplaint(?Customer $customer, string $title, string $category, string $priority, string $status, ?User $staff, int $comments): void
    {
        if (! $customer) {
            return;
        }
        ...
        $complaint = CustomerComplaint::updateOrCreate(
            ...
            [
                ...
                'status' => $status,
                'attachment_path' => $comments > 2 ? 'dummy/complaints/photo-'.$customer->id.'.jpg' : null,
