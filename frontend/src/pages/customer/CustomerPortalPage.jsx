...
function ManualProofDrawer({ payment, open, onClose }) {
  ...
  const upload = useMutation({
    mutationFn: (values) => {
      ...
      return api.customer.uploadManualProof(payment.id, formData);
    },
    onSuccess: () => {
      message.success('Bukti pembayaran dikirim dan menunggu verifikasi.');
      queryClient.invalidateQueries({ queryKey: ['customer-payments'] });
      queryClient.invalidateQueries({ queryKey: ['customer-payment', payment?.id] });
      form.resetFields();
      onClose();
    },
    onError: (error) => {
...
function useCustomerPaymentActions() {
  const navigate = useNavigate();
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: ({ invoiceId, provider }) => api.customer.createPayment(invoiceId, { provider }),
    onSuccess: (response) => {
      ...
      if (payment.payment_url) {
        ...
        navigate(`/customer/payments/${payment.id}`);
      }
    },
...
function CustomerDashboard() {
  const navigate = useNavigate();
  const query = useQuery({ queryKey: ['customer-dashboard'], queryFn: api.customer.dashboard });
  ...
  return (
    <section>
      <PageHeader title="Dashboard Customer" subtitle="Ringkasan akun, unit, tagihan, pembayaran, layanan, dan aktivitas Anda." breadcrumbs={[{ label: 'Customer' }, { label: 'Dashboard' }]} onRefresh={query.refetch} />
      <div className="customer-grid customer-grid-4">
      ...
      <div className="customer-grid customer-grid-2 section-row">
        <Card title="Informasi Customer"><InfoList data={{ Nama: data.customer?.name, Email: data.customer?.email, Telepon: data.customer?.phone, 'Nomor Customer': data.customer?.customer_number, Status: data.customer?.status }} /></Card>
      ...
      <Card className="section-row">
        <Space wrap>
          <Button type="primary" onClick={() => navigate('/customer/bills')}>Pembayaran Cepat</Button>
          <Button onClick={() => navigate('/customer/complaints')} icon={<PlusOutlined />}>Buat Komplain</Button>
          <Button onClick={() => navigate('/customer/maintenance-requests')} icon={<PlusOutlined />}>Buat Maintenance</Button>
          <Button onClick={() => navigate('/customer/bills')}>Lihat Tagihan</Button>
          <Button onClick={() => navigate('/customer/payments')}>Riwayat Pembayaran</Button>
        </Space>
      </Card>
      <div className="customer-grid customer-grid-3 section-row">
        <Card title="Tagihan Terbaru">{asList(billing.latest).map((item) => <p key={item.id}><Button type="link" onClick={() => navigate(`/customer/invoices/${item.id}`)}>{item.invoice_number}</Button> {formatCurrency(item.total)} <StatusBadge type="transaction" value={item.status} /></p>) || <EmptyData />}</Card>
        <Card title="Pembayaran Terbaru">{asList(payment.latest).map((item) => <p key={item.id}><Button type="link" onClick={() => navigate(`/customer/payments/${item.id}`)}>{item.transaction_number}</Button> {formatCurrency(item.total)} <StatusBadge type="transaction" value={item.status} /></p>)}</Card>
...
function ProfileForm({ initial, onSaved }) {
  ...
  const update = useMutation({
    mutationFn: (values) => {
      ...
      return api.customer.updateProfile(formData);
    },
    onSuccess: (response) => {
...
function CustomerAccount({ mode = 'account' }) {
  const query = useQuery({ queryKey: ['customer-account'], queryFn: api.customer.account });
  const data = unwrapQuery(query) || {};
  ...
  if (mode === 'profile') {
    return (
      <section>
        <PageHeader title="Profil Customer" breadcrumbs={[{ label: 'Customer' }, { label: 'Profil' }]} onRefresh={query.refetch} />
  ...
  return (
    <section>
      <PageHeader title="Akun Customer" subtitle="Informasi akun, profil, unit, tagihan, pembayaran, dokumen, keamanan, notifikasi, dan aktivitas." breadcrumbs={[{ label: 'Customer' }, { label: 'Akun' }]} onRefresh={query.refetch} />
      <Tabs
        items={[
          { key: 'account', label: 'Informasi Akun', children: <Card><InfoList data={{ Nama: data.account?.name, Email: data.account?.email, Telepon: data.account?.phone, 'Nomor Customer': data.account?.customer_number, Status: data.account?.status, Estate: data.account?.estate, Unit: data.account?.unit, Bergabung: formatDate(data.account?.joined_at), 'Login Terakhir': formatDateTime(data.account?.last_login_at) }} /></Card> },
          { key: 'profile', label: 'Profil Customer', children: <Card><ProfileForm initial={data.account} /></Card> },
...
function CustomerProperty() {
  const query = useQuery({ queryKey: ['customer-property'], queryFn: api.customer.property });
  const data = unwrapQuery(query) || {};
  return (
    <section>
      <PageHeader title="Properti atau Unit" breadcrumbs={[{ label: 'Customer' }, { label: 'Properti' }]} onRefresh={query.refetch} />
      ...
      <ResponsiveTable
        ...
        columns={[
          ...
          { title: 'Status', dataIndex: 'status', render: (value) => <StatusBadge type="transaction" value={value} />, width: 150 },
          {
            ...
            render: (_, row) => (
              <Space>
                <Button size="small" icon={<EyeOutlined />} onClick={() => navigate(`/customer/invoices/${row.id}`)}>Detail</Button>
                <Button size="small" type="primary" disabled={!['unpaid', 'overdue'].includes(row.status)} onClick={() => onPay?.(row)}>Bayar</Button>
                <Button size="small" icon={<DownloadOutlined />} onClick={async () => downloadBlob(await api.customer.downloadInvoice(row.id), `${row.invoice_number}.pdf`)}>PDF</Button>
              </Space>
            ),
          },
        ]}
...
function CustomerBills() {
  ...
  const query = useQuery({ queryKey: ['customer-bills', table.params], queryFn: () => api.customer.bills(table.params) });
  const config = useQuery({ queryKey: ['customer-payment-config'], queryFn: api.customer.paymentConfig });
  const createPayment = useCustomerPaymentActions();
  ...
  return (
    <section>
      <PageHeader title="Tagihan Customer" subtitle="Cari, filter, sortir, dan bayar invoice milik Anda." breadcrumbs={[{ label: 'Customer' }, { label: 'Tagihan' }]} onRefresh={query.refetch} />
      <FilterBar>
...
function CustomerInvoiceDetail() {
  ...
  const query = useQuery({ queryKey: ['customer-invoice', invoiceId], queryFn: () => api.customer.invoice(invoiceId), enabled: Boolean(invoiceId) });
  const config = useQuery({ queryKey: ['customer-payment-config'], queryFn: api.customer.paymentConfig });
  const createPayment = useCustomerPaymentActions();
  const data = unwrapQuery(query) || {};
  const configData = unwrapQuery(config);
  ...
  return (
    <section>
      <PageHeader title="Detail Invoice" breadcrumbs={[{ label: 'Customer' }, { label: 'Invoice' }, { label: data.invoice_number || invoiceId }]} onRefresh={query.refetch} />
      {query.isLoading ? <LoadingState rows={8} /> : query.isError ? <ErrorState error={query.error} onRetry={query.refetch} /> : (
        <div className="customer-grid customer-grid-2">
          ...
          <Card title="Riwayat Pembayaran" className="customer-wide"><PaymentTable data={data.payment_history} /></Card>
        </div>
      )}
    </section>
  );
        ...
        <ResponsiveTable
          ...
          columns={[
            ...
            { title: 'Kedaluwarsa', dataIndex: 'expired_at', render: formatDateTime, width: 170 },
            {
              ...
              render: (_, row) => (
                <Space>
                  <Button size="small" icon={<EyeOutlined />} onClick={() => navigate(`/customer/payments/${row.id}`)}>Detail</Button>
                  {row.status === 'paid' ? <Button size="small" icon={<DownloadOutlined />} onClick={async () => downloadBlob(await api.customer.downloadPaymentReceipt(row.id), `${row.transaction_number}.pdf`)}>Receipt</Button> : null}
...
function CustomerPayments() {
  const table = useTableState();
  const query = useQuery({ queryKey: ['customer-payments', table.params], queryFn: () => api.customer.payments(table.params) });
  return (
    <section>
      <PageHeader title="Riwayat Pembayaran" breadcrumbs={[{ label: 'Customer' }, { label: 'Pembayaran' }]} onRefresh={query.refetch} />
      <FilterBar>
...
function CustomerPaymentDetail() {
  ...
  const query = useQuery({ queryKey: ['customer-payment', paymentId], queryFn: () => api.customer.payment(paymentId), enabled: Boolean(paymentId) });
  const [proofOpen, setProofOpen] = useState(false);
  const data = unwrapQuery(query) || {};
  return (
    <section>
      <PageHeader title="Detail Pembayaran" breadcrumbs={[{ label: 'Customer' }, { label: 'Pembayaran' }, { label: data.transaction_number || paymentId }]} onRefresh={query.refetch} />
      {query.isLoading ? <LoadingState rows={8} /> : query.isError ? <ErrorState error={query.error} onRetry={query.refetch} /> : (
        <div className="customer-grid customer-grid-2">
          <Card title={data.transaction_number}>
            <InfoList data={{ Invoice: data.invoice_number, Customer: data.customer?.name, Gateway: data.payment_gateway, Metode: data.payment_method, Subtotal: formatCurrency(data.fee_breakdown?.subtotal), Pajak: formatCurrency(data.fee_breakdown?.tax), Admin: formatCurrency(data.fee_breakdown?.admin_fee), Total: formatCurrency(data.fee_breakdown?.total), Status: data.status, 'Waktu transaksi': formatDateTime(data.created_at), 'Waktu pembayaran': formatDateTime(data.paid_at), 'Kedaluwarsa': formatDateTime(data.expired_at), 'Bukti manual': data.manual_proof_path, 'Catatan verifikator': data.verification_notes }} />
            <Space className="action-row" wrap>
              {data.status === 'paid' ? <Button icon={<DownloadOutlined />} onClick={async () => downloadBlob(await api.customer.downloadPaymentReceipt(data.id), `${data.transaction_number}.pdf`)}>Download Receipt</Button> : null}
...
function CustomerPaymentMethods() {
  const query = useQuery({ queryKey: ['customer-payment-config'], queryFn: api.customer.paymentConfig });
  return (
    <section>
      <PageHeader title="Metode Pembayaran" breadcrumbs={[{ label: 'Customer' }, { label: 'Metode Bayar' }]} onRefresh={query.refetch} />
...
function ComplaintMaintenancePage({ kind }) {
  ...
  const endpoint = isComplaint ? api.customer.complaints : api.customer.maintenanceRequests;
  const createEndpoint = isComplaint ? api.customer.createComplaint : api.customer.createMaintenanceRequest;
  const detailEndpoint = isComplaint ? api.customer.complaint : api.customer.maintenanceRequest;
  const query = useQuery({ queryKey: [`customer-${kind}`, table.params], queryFn: () => endpoint(table.params) });
  const detailQuery = useQuery({ queryKey: [`customer-${kind}-detail`, detail?.id], queryFn: () => detailEndpoint(detail.id), enabled: Boolean(detail?.id) });
  ...
  const create = useMutation({
    ...
    onSuccess: () => {
      ...
      setOpen(false);
      form.resetFields();
      queryClient.invalidateQueries({ queryKey: [`customer-${kind}`] });
    },
    onError: (error) => {
      form.setFields(mapValidationErrors(error));
      message.error(getApiErrorMessage(error));
  ...
  const addComment = useMutation({
    mutationFn: (values) => {
      ...
      return api.customer.addComplaintComment(detail.id, formData);
    },
    onSuccess: () => {
      message.success('Komentar dikirim.');
      commentForm.resetFields();
      queryClient.invalidateQueries({ queryKey: [`customer-${kind}-detail`, detail?.id] });
    },
    onError: (error) => message.error(getApiErrorMessage(error)),
  });

  const closeComplaint = useMutation({
    mutationFn: () => api.customer.closeComplaint(detail.id),
    onSuccess: () => {
  ...
  const rate = useMutation({
    mutationFn: (values) => api.customer.rateMaintenanceRequest(detail.id, values),
    onSuccess: () => {
  ...
  return (
    <section>
      <PageHeader
        title={isComplaint ? 'Komplain Customer' : 'Maintenance Request'}
        breadcrumbs={[{ label: 'Customer' }, { label: isComplaint ? 'Komplain' : 'Maintenance' }]}
...
function DocumentList({ documents, query }) {
  ...
  async function download(row) {
    const blob = row.download_type === 'payment_receipt'
      ? await api.customer.downloadPaymentReceipt(row.download_id)
      : row.download_type === 'receipt'
        ? await api.customer.downloadCashReceipt(row.download_id)
        : await api.customer.downloadInvoice(row.download_id);
...
function CustomerDocuments() {
  const query = useQuery({ queryKey: ['customer-documents'], queryFn: api.customer.documents });
  return (
    <section>
      <PageHeader title="Dokumen Customer" breadcrumbs={[{ label: 'Customer' }, { label: 'Dokumen' }]} onRefresh={query.refetch} />
  ...
  return (
    <Card>
      {items.length ? items.map((item) => (
        <div className="customer-list-item" key={item.id}>
          <div>
...
function CustomerNotifications() {
  const table = useTableState();
  const queryClient = useQueryClient();
  const query = useQuery({ queryKey: ['customer-notifications', table.params], queryFn: () => api.customer.notifications(table.params) });
  const read = useMutation({ mutationFn: api.customer.readNotification, onSuccess: () => queryClient.invalidateQueries({ queryKey: ['customer-notifications'] }) });
  const readAll = useMutation({ mutationFn: api.customer.readAllNotifications, onSuccess: () => queryClient.invalidateQueries({ queryKey: ['customer-notifications'] }) });
  return (
    <section>
      <PageHeader title="Notifikasi Customer" breadcrumbs={[{ label: 'Customer' }, { label: 'Notifikasi' }]} actions={<Button icon={<CheckOutlined />} onClick={() => readAll.mutate()} loading={readAll.isPending}>Tandai Semua Dibaca</Button>} onRefresh={query.refetch} />
      <FilterBar>
...
function CustomerActivity() {
  const table = useTableState();
  const query = useQuery({ queryKey: ['customer-activity', table.params], queryFn: () => api.customer.activity(table.params) });
  return (
    <section>
      <PageHeader title="Aktivitas Customer" breadcrumbs={[{ label: 'Customer' }, { label: 'Aktivitas' }]} onRefresh={query.refetch} />
...
function CustomerSettings() {
  const queryClient = useQueryClient();
  const { setMode } = useThemeMode();
  const query = useQuery({ queryKey: ['customer-settings'], queryFn: api.customer.settings });
  const data = unwrapQuery(query) || {};
  const [form] = Form.useForm();
  useEffect(() => {
    form.setFieldsValue({
  ...
  const update = useMutation({
    mutationFn: (values) => api.customer.updateSettings({
    ...
    onSuccess: (_, values) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['customer-settings'] });
    },
    onError: (error) => message.error(getApiErrorMessage(error)),
  });
  ...
  return (
    <section>
      <PageHeader title="Pengaturan Customer" breadcrumbs={[{ label: 'Customer' }, { label: 'Pengaturan' }]} onRefresh={query.refetch} />
      <Card>
...
export default function CustomerPortalPage({ page }) {
  const screens = Grid.useBreakpoint();
  const current = useMemo(() => page, [page]);
  useEffect(() => {
    document.body.classList.toggle('customer-mobile', !screens.md);
    return () => document.body.classList.remove('customer-mobile');
  }, [screens.md]);

  if (current === 'dashboard') return <CustomerDashboard />;
  if (current === 'account') return <CustomerAccount />;
  if (current === 'profile') return <CustomerAccount mode="profile" />;
  if (current === 'property') return <CustomerProperty />;
  if (current === 'bills' || current === 'invoices') return <CustomerBills />;
  if (current === 'invoice-detail') return <CustomerInvoiceDetail />;
  if (current === 'payments') return <CustomerPayments />;
  if (current === 'payment-detail') return <CustomerPaymentDetail />;
  if (current === 'payment-methods') return <CustomerPaymentMethods />;
  ...
  if (current === 'documents') return <CustomerDocuments />;
  if (current === 'notifications') return <CustomerNotifications />;
  if (current === 'activity') return <CustomerActivity />;
  if (current === 'settings') return <CustomerSettings />;
  return <EmptyData description="Halaman customer tidak ditemukan." />;
}
